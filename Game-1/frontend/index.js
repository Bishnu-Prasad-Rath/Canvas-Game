...
const API_BASE = isLocalhost 
  ? "http://localhost:3000" 
  : "https://canvas-game-v2.onrender.com"; // Update this to your new Render URL later!
...
async function loadLeaderboard() {
  try {
    ...
    data.forEach((item, index) => {
      ...
      li.className = `flex items-center justify-between px-4 py-2 rounded-lg bg-white/5 hover:bg-white/10 transition ${rankColors[index] || "text-gray-300"}`;
      li.innerHTML = `
        <div class="flex items-center gap-3">
          <span class="font-bold w-4">${index + 1}</span>
          <span class="truncate max-w-[140px]">${item.username}</span>
        </div>
        <span class="font-extrabold text-green-400">${item.score}</span>
      `;
...
async function saveScore(score) {
  ...
  try {
    ...
    if (!res.ok) console.warn("Score not saved:", res.status);
  } catch (err) {
    console.error("Score save failed:", err);
  }
}

loadLeaderboard(); // Load immediately on page load
    ...
    projectiles.forEach((projectile, projectileIndex) => {
      ...
      if (dist - enemy.radius - projectile.radius < 1) {
        ...
        if (enemy.radius - 10 > 10) {
          score += 100;
          // Fallback just in case GSAP isn't linked in your HTML
          if(window.gsap) {
...
addEventListener('click', (event) => {
  // Prevent shooting when clicking UI buttons
  if(event.target.closest('#authModal') || event.target.closest('#topBar') || event.target.closest('#modelEl') || event.target.closest('#homePage')) {
      return;
  }
